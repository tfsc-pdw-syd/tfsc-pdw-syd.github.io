...
function isTestRecord(record) {
  ...
  const testTitles = ['test', 'testing', 'test submission', 'test paper', 'test abstract'];
  return (
    testTitles.includes(title) ||
    filename === 'test.pdf' ||
...
function getAdminRecords() {
  ...
  // Deduplicate by title (case-insensitive), keeping the most recent submission
  const seen = new Map();
  for (const record of records) {
    const key = String(record.title || '').trim().toLowerCase();
    seen.set(key, record);
  }
